...
lambda_function_name=$(aws cloudformation describe-stacks --stack-name ipv-cri-otg-hmrc-smoke-tests --query "Stacks[0].Outputs[?OutputKey=='CanaryInvokerFunction'].OutputValue" --output text)
canary_names=("get-bearer-token")
